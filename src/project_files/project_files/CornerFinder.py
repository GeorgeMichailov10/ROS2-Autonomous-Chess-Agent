...
class CornerFinder(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(GetCorners, 'get_board_corners', self.getCornersCallback)
        # BGR NOT RGB
        # Some of these ranges are redundant, but all are necessary.
        self.lower_pink1 = np.array([110, 25, 200], dtype=np.uint8)
        self.upper_pink1 = np.array([255, 80, 255], dtype=np.uint8)

        self.lower_pink2 = np.array([140, 80, 200], dtype=np.uint8)
        self.upper_pink2 = np.array([255, 132, 255], dtype=np.uint8)

        self.lower_pink3 = np.array([170, 130, 220], dtype=np.uint8)
        self.upper_pink3 = np.array([255, 165, 255], dtype=np.uint8)

        self.lower_pink4 = np.array([140, 110, 200], dtype=np.uint8)
        self.upper_pink4 = np.array([255, 150, 255], dtype=np.uint8)

        self.lower_pink5 = np.array([0, 0, 200], dtype=np.uint8)
        self.upper_pink5 = np.array([100, 100, 255], dtype=np.uint8)
    ...
    def getCorners(self, board_image):
        ...
        # Search for each corner in its respect quadrant
        ...
        # Get actual coordinates because current coordinates are relative to the quadrant
        tl_corner = self.find_top_left_corner(top_left_quad)
        tr_corner = self.find_top_right_corner(top_right_quad)
        tr_corner[1] += width // 2
        bl_corner = self.find_bottom_left_corner(bottom_left_quad)
        bl_corner[0] += height // 2
        br_corner = self.find_bottom_right_corner(bottom_right_quad)
    ...
    def find_top_left_corner(self, quadrant):
        corner = None
    ...
    def find_top_right_corner(self, quadrant):
        corner = None
        for row in range(0, quadrant.shape[0], self.skip_steps):
    ...
    def find_bottom_left_corner(self, quadrant):
        corner = None
    ...
    def find_bottom_right_corner(self, quadrant):
        corner = None
        for row in range(quadrant.shape[0] - 1, -1, -1 * self.skip_steps):
            for col in range(quadrant.shape[1] - 1, -1, -1 * self.skip_steps):
                if self.is_pink(quadrant[row, col]):
    ...
    def is_pink(self, pixel):
        return (
            (self.lower_pink1[0] <= pixel[0] <= self.upper_pink1[0] and self.lower_pink1[1] <= pixel[1] <= self.upper_pink1[1] and self.lower_pink1[2] <= pixel[2] <= self.upper_pink1[2]) or 
            (self.lower_pink2[0] <= pixel[0] <= self.upper_pink2[0] and self.lower_pink2[1] <= pixel[1] <= self.upper_pink2[1] and self.lower_pink2[2] <= pixel[2] <= self.upper_pink2[2]) or 
            (self.lower_pink3[0] <= pixel[0] <= self.upper_pink3[0] and self.lower_pink3[1] <= pixel[1] <= self.upper_pink3[1] and self.lower_pink3[2] <= pixel[2] <= self.upper_pink3[2]) or
            (self.lower_pink4[0] <= pixel[0] <= self.upper_pink4[0] and self.lower_pink4[1] <= pixel[1] <= self.upper_pink4[1] and self.lower_pink3[2] <= pixel[2] <= self.upper_pink4[2]) or
            (self.lower_pink5[0] <= pixel[0] <= self.upper_pink5[0] and self.lower_pink5[1] <= pixel[1] <= self.upper_pink5[1] and self.lower_pink5[2] <= pixel[2] <= self.upper_pink5[2])
        )
