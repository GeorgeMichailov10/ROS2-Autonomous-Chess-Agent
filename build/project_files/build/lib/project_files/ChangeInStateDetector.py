...
class BoardStateDetector(Node):
    def __init__(self):
        ...
        self.cli = self.create_client(GetCorners, 'get_board_corners')                                            # Become client to get corners service node
        self.br = CvBridge()
    ...
    def generateBitMap(self):
        ...
        for row in range(8):
            ...
            for col in range(8):
                ...
                bitmap[row].append(self.isOccupied(square))
        return bitmap
    ...
    def isOccupied(self, square):
        ...
        for row in range(0, height, 4):
            for col in range(0, width, 4):
                if self.isGreen(square[row][col]):
                    count+=1
            if count >= 10:
                return 'o'
        if count >= 10:
